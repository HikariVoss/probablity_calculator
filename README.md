...
Then using a for loop like this: `for _ in range(num_experiments)` it loops for the number of desired experiments. Then it takes a deep copy of the hat, to avoid accidental modification of the original hat and draws the number of balls. By using `hat_copy.contents.pop(random.randrange(len(hat_copy.contents))) for _ range(num_balls_drawn)`. Then taking the balls drawn and comparing them to the `expected_balls` dictionary we can find the number of successful draws.
